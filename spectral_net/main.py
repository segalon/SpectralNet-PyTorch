...
def train(model, params, dataset_train):
  ...
  if to_wandb:
    wandb.init(project="SpectralNet2", entity="bkw1", config=params)
  ...
  for epoch in range(params['n_epochs'] + 1):
    ...
    if epoch % print_every == 0:
      with torch.no_grad():
        acc = get_cluster_acc(model, dataset_train, k=k)
...
params = {
  ...
  'print_every': 15,
  'log_every': 5,
  'path': "/content/drive/MyDrive/geometry_information/geoi_project/model_orhto_layer.pt",
  'dataset': "cc",
  'to_wandb': False,
  'device': device
}
