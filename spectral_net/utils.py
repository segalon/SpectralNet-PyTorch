...
def get_cluster_acc(model, data, k, input_sz):
  N = len(data)
...
def plot_clustering(model, x_train, k):
    preds = model(torch.tensor(x_train))
    km = KMeans(n_clusters=k, random_state=20)
    cluster_preds = km.fit_predict(preds.detach().numpy())
    df = pd.DataFrame({"x":x_train[:,0], "y":x_train[:, 1], "cluster": cluster_preds})
